...
# Read Xtable
Xtab = pd.read_csv(xfile, sep=",", header=None, names=('a','b','c'), dtype={'a':'str','b':'int','c':'int'})
    ...
    for fas in falist:
        ...
        if not Xsub.empty:
            ...
            for row in Xsub.itertuples(index=True):
                # From where to where
                Xfrom = min(int(getattr(row, 'b')), int(getattr(row, 'c')))
                Xto = max(int(getattr(row, 'b')), int(getattr(row, 'c')))
                # New sequence
